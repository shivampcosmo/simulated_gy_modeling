...
class ProfileBase:
	...
	def __init__(self, cosmology=None, params_update=None, broken_pl=False):
	...
	def get_params_battaglia(self, m200, z, cosmology): raise NotImplementedError
...
class ProfileBattaglia(ProfileBase):
	...
	def get_params_battaglia(self, m200, z, cosmology):
		...
		if self.params_update is not None:
			P0_A, P0_alm, P0_alz = self.params_update.P0_A, self.params_update.P0_alm, self.params_update.P0_alz
			xc_A, xc_alm, xc_alz = self.params_update.xc_A, self.params_update.xc_alm, self.params_update.xc_alz
			beta_A, beta_alm, beta_alz = self.params_update.beta_A, self.params_update.beta_alm, self.params_update.beta_alz
			if self.broken_pl:
				bpl_alm = self.params_update.bpl_alm
		else:
			P0_A, P0_alm, P0_alz = 18.1, 0.154, -0.758
			xc_A, xc_alm, xc_alz = 0.497, -0.00865, 0.731
			beta_A, beta_alm, beta_alz = 4.35, 0.0393, 0.415
			
		
		bpl_mod = np.ones_like(m)
		if self.broken_pl:
			mbreak_kg = self.params_update.Mbreak*utils.M_sun/cosmology['h']
			indsel = np.where(m200 < mbreak_kg)[0]						
			mpl = m200[indsel]/mbreak_kg
			bpl_mod[indsel] = mpl**bpl_alm

		P0 = bpl_mod * P0_A * m**P0_alm * z1**P0_alz
		xc = xc_A * m**xc_alm * z1**xc_alz
		beta = beta_A * m**beta_alm * z1**beta_alz
		alpha =  1; gamma = -0.3
	...
	def _raw(self, m200, z, r, dist="physical", r200=None):
		...
		params = self.get_params_battaglia(m200, z, self.cosmology)
...
class ProfileBattagliaFast(ProfileBattaglia):
	def __init__(self, cosmology=None, beta_range=[-14,-3], obeta=6, nbeta=None, pad=0.1,
			alpha=1, gamma=-0.3, x1=1e-10, x2=1e8, npoint=200, zmax=1e5, _a=8, params_update=None, 
			broken_pl=False):
...
def websky_decode(data, cosmology, mass_interp, lgMmin=None):
	...
	if lgMmin is not None:
		...
		z, ra, dec, m200 = z[indsel], ra[indsel], dec[indsel], m200[indsel]
